<template>
  <div id="app">
    ...
    <Footer :user-count="Object.keys(users).length"
            :post-count="Object.keys(posts).length"/>
  </div>
</template>
...
export default {
  ...
  beforeCreate: function () {
    ...
    this.$root.$on("onRegister", (login, name) => {
      ...
      } else if (!/^[a-z]{3,16}$/.test(login)) {
        this.$root.$emit("onRegisterValidationError", "Only latin lowercase letter allow, length between 3 and 16!")
      } else if (!name) {
      ...
      } else if (Object.values(this.users).find(it => it.login === login)) {
    ...
    this.$root.$on("onShowPost", (postId) => {
