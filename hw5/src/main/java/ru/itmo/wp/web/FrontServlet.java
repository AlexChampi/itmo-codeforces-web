...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        for (Class<?> clazz = pageClass; !optionalMethod.isPresent() && clazz != null; clazz = clazz.getSuperclass()) {
            optionalMethod = Arrays.stream(clazz.getDeclaredMethods()).filter(meth -> meth.getName().equals(route.action)).findFirst();
        }
        ...
        try {
            Object[] methodParameters = getMethodParameters(method.getParameterTypes(), request, view);
            method.invoke(page, methodParameters);
        ...
        setLocale(request);
        String lang = "";
        if (Objects.nonNull(request.getSession().getAttribute("lang"))) {
            lang = (String) request.getSession().getAttribute("lang");
        }

        Template template = newTemplate(pageClass.getSimpleName() + ".ftlh", new Locale(lang));
    ...
    private void setLocale(HttpServletRequest request) {
        ...
        if (Objects.nonNull(lang) && lang.length() == 2 && lang.toLowerCase().equals(lang)) {
            request.getSession().setAttribute("lang", lang);
        }
    }

    private Object[] getMethodParameters(Class<?>[] parameterTypes, HttpServletRequest request, Map<String, Object> view) {
